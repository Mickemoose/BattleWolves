	...
	Pressed(k)
		...
		if(usr.client.input_lock == 1)
			...
			if(!usr.canAct)
				...
				if(usr.inSSS)
					if(k == "GamepadRight" && Stage_Selected==null)
						usr<<CLICK
						usr.sssicon++
						if(usr.sssicon > usr.sssicons.len)
							usr.sssicon=1
						for(var/UI/CSS/Cursor/C in usr.cursor)
							C.screen_loc=usr.sssicons[usr.sssicon].screen_loc
						//setTempPortrait(stages[sssicon])
						return
					if(k == "GamepadLeft" && Stage_Selected==null)
						usr<<CLICK
						usr.sssicon--
						if(usr.sssicon <= 0)
							usr.sssicon=usr.sssicons.len
						for(var/UI/CSS/Cursor/C in usr.cursor)
							C.screen_loc=usr.sssicons[usr.sssicon].screen_loc
						//setTempPortrait(stages[sssicon])
						return
					if(k == "GamepadFace2" && Stage_Selected==null)
						for(var/UI/CSS/Cursor/C in usr.cursor)
							C.screen_loc=usr.cssicons[usr.cssicon].screen_loc
						usr.inSSS=0
						usr.inCSS=1
						usr<<CANCEL
						//return
					if(k == "GamepadStart" || k  == "GamepadFace1" && Stage_Selected==null)
						usr<<CHOOSE
						Stage_Selected="[usr.sssicons[usr.sssicon]]"
				...
				if(usr.inCSS)
					if(k == "GamepadRight" && usr.character==null)
						usr<<CLICK
						usr.cssicon++
						if(usr.cssicon > usr.cssicons.len)
							usr.cssicon=1
						for(var/UI/CSS/Cursor/C in usr.cursor)
							C.screen_loc=usr.cssicons[usr.cssicon].screen_loc
						usr.setTempPortrait(usr.characters[usr.cssicon])
						return
					if(k == "GamepadLeft" && usr.character==null)
						usr<<CLICK
						usr.cssicon--
						if(usr.cssicon <= 0)
							usr.cssicon=usr.cssicons.len
						for(var/UI/CSS/Cursor/C in usr.cursor)
							C.screen_loc=usr.cssicons[usr.cssicon].screen_loc
						usr.setTempPortrait(usr.characters[usr.cssicon])
						return
